...
# 意図したディスプレイサイズ(後に設定するサイズを最適に表示できるサイズ≒4K)でない場合は何もしない
...
# ディスプレイを設定してミラーする（既に設定済みの場合は何もしない）
config="id:$external_display_persistent_id+$macbook_display_persistent_id res:2560x1440 hz:60 color_depth:8 enabled:true scaling:on origin:(0,0) degree:0"
